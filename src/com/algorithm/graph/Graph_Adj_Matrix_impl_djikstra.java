...
public class Graph_Adj_Matrix_impl_djikstra {
    private Integer[][] adj_matrix;
    private char[] vertex_states;

    public Graph_Adj_Matrix_impl_djikstra (int vertex_num) {
        ...
        for (int row = 0 ; row < adj_matrix.length ; row ++ ) {
            for ( int col = 0 ; col < adj_matrix[row].length ; col ++ ) {
                this.adj_matrix[row][col] = Integer.MAX_VALUE;
            }
        }
    ...
    public void print_adj_matrix () {
        ...
        for (int row = 0 ; row < adj_matrix.length ; row ++ ) {
            ...
            for ( int col = 0 ; col < adj_matrix[row].length ; col ++ ) {
                if ( adj_matrix[row][col] == Integer.MAX_VALUE ) {
                    System.out.printf("∞" + " ");
                }
                else {
    ...
    public void find_shortest_path_djikstra (int i_vertex_start) {
        ...
        while ( true ) {
            ...
            for ( int i = 0 ; i < neighbors.length ; i++ ) {
                ...
                if ( distance_to_add == Integer.MAX_VALUE ) {
                    continue;
                }
    ...
    public static void main (String[] args) {
        Graph_Adj_Matrix_impl_djikstra graph_adj_matrix_impl_distance_plus_vertex_state = new Graph_Adj_Matrix_impl_djikstra(6);
