...
public class BST_234Tree_List {

    public static class Node_234Tree {
        Node_234Tree parent;
        LinkedList<Integer> keys = new LinkedList<>();  // nodes_max = 3
        LinkedList<Node_234Tree> children = new LinkedList<>();  // children_max = 4
    }

    private int[] nums;
    private Node_234Tree root;

    public BST_234Tree_List (int[] nums) {
        this.nums = nums;
    }
    ...
    private void add (int num) {
        if ( root == null ) {
            root = new Node_234Tree();
            root.keys.add(num);
        }
        else {
            adjust_tree(root, num);
            add_helper(root, num);
        }
    }

    private void adjust_tree (Node_234Tree node, int num_to_add) {
        if ( node.children.isEmpty() ) {
            if ( node.keys.size() == 3 ) {
                adjust_tree_helper(node);
            }
            return;
        }
        ...
        if ( node.keys.size() == 1 ) {
            ...
            if ( num_to_add < key ) {
                adjust_tree(node.children.getFirst(), num_to_add);
            }
            else if ( num_to_add > key ) {
                adjust_tree(node.children.getLast(), num_to_add);
            }
            else {  // num_to_add == key
                // ignore same value
            }
            ...
            if ( num_to_add < key_left ) {
                ...
                adjust_tree(node.children.get(1), num_to_add);
            }
            ...
            int key_left = node.keys.getFirst();
            int key_middle = node.keys.get(1);
            int key_right = node.keys.getLast();

            if ( num_to_add < key_left ) {
                adjust_tree(node.children.getFirst(), num_to_add);
            }
            else if ( num_to_add > key_left && num_to_add < key_middle ) {
                adjust_tree(node.children.get(1), num_to_add);
            }
            else if ( num_to_add > key_middle && num_to_add < key_right ) {
                adjust_tree(node.children.get(2), num_to_add);
            }
            else if ( num_to_add > key_right ) {
                adjust_tree(node.children.getLast(), num_to_add);
            }
    ...
    private void adjust_tree_helper (Node_234Tree node) {
        if ( node.parent != null && node.parent.keys.size() == 3 ) {
            adjust_tree_helper(node.parent);
        }
        break_to_merge(node);
    }

    private void add_helper (Node_234Tree node, int num_to_add) {
        if ( node.children.isEmpty() ) {
        ...
        if ( node.keys.size() == 1 ) {
            ...
            if ( num_to_add < key_left ) {
                ...
                add_helper(node.children.get(2), num_to_add);
            }
            else if ( num_to_add > key_right ) {
                add_helper(node.children.getLast(), num_to_add);
            }
    ...
    private void break_to_merge (Node_234Tree node) {
        // break: a [3 - key node] to [1 - key node + 2 children node]
        break_nodes(node);

        // merge with parent
        if ( node.parent != null ) {
            Node_234Tree node_parent_after_merge = merge_to_parent(node);
    ...
    private Node_234Tree merge_to_parent (Node_234Tree node) {
        if ( node.parent == null ) return node;

        Node_234Tree node_parent = node.parent;
        // remove this child
    ...
    private void merge_to_parent_helper (Node_234Tree node_parent, Node_234Tree node_to_merge) {
    ...
    private void remove_child (Node_234Tree node_parent, Node_234Tree node) {
        for ( int i = 0 ; i < node_parent.children.size() ; i++ ) {
            Node_234Tree child = node_parent.children.get(i);
    ...
    private void break_nodes (Node_234Tree node) {
        Node_234Tree node_left = new Node_234Tree();
        Node_234Tree node_right = new Node_234Tree();
        ...
        for ( int i = 0 ; i < node.children.size() ; i++ ) {
            Node_234Tree child_now = node.children.get(i);
    ...
    private Integer search_helper (Node_234Tree node, int target) {
        if ( node.keys.size() == 1 ) {
            ...
            else if ( target > key_left && target < key_middle ) {
                return search_helper(node.children.get(1), target);
            }
            else if ( target > key_middle && target < key_right ) {
                return search_helper(node.children.get(2), target);
            }
            else if ( target > key_right ) {
                return search_helper(node.children.getLast(), target);
            }
    ...
    public static void main (String[] args) {
        int[] nums = new int[]{39, 28, 17, 13, 11, 4, 3, 7, 12, 9};

        BST_234Tree_List tree234 = new BST_234Tree_List(nums);
        tree234.build_tree();

        Integer target_num = null;
        target_num = tree234.search(17);
        target_num = tree234.search(100);
